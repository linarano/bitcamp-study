...
-- 강사
DROP TABLE IF EXISTS lma_fchr RESTRICT;

-- 수강생
DROP TABLE IF EXISTS lms_stud RESTRICT;

-- 매니저
DROP TABLE IF EXISTS lms_mger RESTRICT;
...
-- 직급
DROP TABLE IF EXISTS lms_positon RESTRICT;
...
-- 수강신청
DROP TABLE IF EXISTS lms_lect_app RESTRICT;
...
CREATE TABLE lms_lecture (
  ...
  contetn  LONGTEXT     NOT NULL COMMENT '강의내용', -- 강의내용
  ...
  day-hr   INTEGER      NULL     DEFAULT 0 COMMENT '일수업시간', -- 일수업시간
...
-- 강사
ALTER TABLE lma_fchr
  ADD CONSTRAINT PK_lma_fchr -- 강사 기본키
    PRIMARY KEY (
      mno -- 강사번호
    );
...
CREATE TABLE lms_stud (
  ...
  sch_nm  VARCHAR(20) NOT NULL COMMENT '최종학교', -- 최종학교
...
-- 매니저
ALTER TABLE lms_mger
  ADD CONSTRAINT PK_lms_mger -- 매니저 기본키
    PRIMARY KEY (
      mrno -- 매니저번호
    );
...
-- 회원 유니크 인덱스3
CREATE UNIQUE INDEX UIX_lms_member3
  ON lms_member ( -- 회원
    tel ASC -- 휴대폰
  );
...
-- 직급
ALTER TABLE lms_positon
  ADD CONSTRAINT PK_lms_positon -- 직급 기본키
    PRIMARY KEY (
      pono -- 직급번호
    );

-- 직급 유니크 인덱스
CREATE UNIQUE INDEX UIX_lms_positon
  ON lms_positon ( -- 직급
    name ASC -- 직급명
  );

ALTER TABLE lms_positon
  MODIFY COLUMN name VARCHAR(20) NOT NULL AUTO_INCREMENT COMMENT '직급명';
...
-- 수강신청
ALTER TABLE lms_lect_app
  ADD CONSTRAINT PK_lms_lect_app -- 수강신청 기본키
    PRIMARY KEY (
      lno, -- 강의번호
      sno  -- 수강생번호
    );
...
-- 강의
ALTER TABLE lms_lecture
  ADD CONSTRAINT FK_lms_mger_TO_lms_lecture -- 매니저 -> 강의
    FOREIGN KEY (
      mrno -- 매니저번호
    )
    REFERENCES lms_mger ( -- 매니저
      mrno -- 매니저번호
    );

-- 강사
ALTER TABLE lma_fchr
  ADD CONSTRAINT FK_lms_member_TO_lma_fchr -- 회원 -> 강사
    FOREIGN KEY (
      mno -- 강사번호
    )
...
-- 강사
ALTER TABLE lma_fchr
  ADD CONSTRAINT FK_lms_work_type_TO_lma_fchr -- 고용형태 -> 강사
    FOREIGN KEY (
      wtno -- 고용형태번호
    )
...
-- 매니저
ALTER TABLE lms_mger
  ADD CONSTRAINT FK_lms_member_TO_lms_mger -- 회원 -> 매니저
    FOREIGN KEY (
      mrno -- 매니저번호
    )
    REFERENCES lms_member ( -- 회원
...
-- 매니저
ALTER TABLE lms_mger
  ADD CONSTRAINT FK_lms_dept_TO_lms_mger -- 부서 -> 매니저
    FOREIGN KEY (
      dno -- 부서번호
    )
...
-- 매니저
ALTER TABLE lms_mger
  ADD CONSTRAINT FK_lms_positon_TO_lms_mger -- 직급 -> 매니저
    FOREIGN KEY (
      pono -- 직급번호
    )
    REFERENCES lms_positon ( -- 직급
      pono -- 직급번호
    );
...
-- 수강신청
ALTER TABLE lms_lect_app
  ADD CONSTRAINT FK_lms_lecture_TO_lms_lect_app -- 강의 -> 수강신청
    FOREIGN KEY (
      lno -- 강의번호
    )
...
-- 수강신청
ALTER TABLE lms_lect_app
  ADD CONSTRAINT FK_lms_stud_TO_lms_lect_app -- 수강생 -> 수강신청
    FOREIGN KEY (
      sno -- 수강생번호
    )
...
-- 강사배정
ALTER TABLE lms_lect_tchr
  ADD CONSTRAINT FK_lma_fchr_TO_lms_lect_tchr -- 강사 -> 강사배정
    FOREIGN KEY (
      mno -- 강사번호
    )
    REFERENCES lma_fchr ( -- 강사
      mno -- 강사번호
    );