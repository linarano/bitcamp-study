...
// stateful 방식
// => 서버와 연결한 후, 
//    보통(원칙) 클라이언트에서 연결을 끊을 때까지(또는 서버에서 연결을 끊을 때까지)
...
// => 작업 시간: 
//    - = 데이터 전송 시간 + 작업 처리 시간 + 데이터 수신 시간 (연결시간이 포함안됨)
...
//    - 채팅 서버: 전용 클라이언트를 이용한 채팅. 예) 유튜브 라이브 방송 채팅 - 서로 컴퓨터가 켜져있는 상태 (메신저는 아님- 한쪽만 켜져있어도 상관없음 서버로 보냄- 카카오톡: 핸드폰을 꺼놓고있다가 키면 한번에 데이터가 들어옴 )
//    - 텔렛: 원격 제어 프로그램 (아마존)
//    - FTP: 파일 전송 프로그램
//    - 오프라인 예: 건강보험문의, 상담 등 , 전화
//
// => 통신
// 클라이언트            서버
//     |------------------>| 연결
