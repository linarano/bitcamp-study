...
public class Receiver {
  ...
  public static void main(String[] args) throws Exception {
    ...
    // - 1~1023 사이의 포트번호는 특정 서버가 사용하기 위해 미리 예약된 번호다. 유명한 프로그램의 포트번호도 가능한 사용하지말라.
    //- 가능한 이범위의 포트 번호는 사용하지않는 것이 좋다. 
    //   - 같은 컴퓨터에서 이미 사용중인 포트 번호는 지정할 수 없다. (중복불가)
    //   - 예) Oracle DBMS(1521), MySQL DBMS(3306) 등
    ServerSocket serverSocket = new ServerSocket(8888); //정상적 실행되면 객체가 실행되어 주소 리턴
    ...
    serverSocket.close();// ㅋ르라이언트의 연결 요청을 받지않는다. 
