// hash code 응용 - HashSet 과 hashCode() - 데이터를 저장/조회/ 삭제할때 해시값으로 데이터 위치를 찾는다. set은 집합으로 같은값 중복저장불가능
...
public class Exam0150 {
  ...
  public static void main(String[] args) {
    ...
    System.out.println(s1 == s2);// 값이 같더라도 서로다른 인스턴스임 

    System.out.println(s1.hashCode());//인스턴스가 다르면 해시코드도 다름 - 오브젝트의 해쉬코드() -오버라이딩 전
    ...
    // 해시셋(집합)에 객체를 보관한다.
    HashSet<Student> set = new HashSet<Student>();//-마치어레이리스트철엄 객체보관
    set.add(s1);
    set.add(s2);
    set.add(s3);
    set.add(s4);
