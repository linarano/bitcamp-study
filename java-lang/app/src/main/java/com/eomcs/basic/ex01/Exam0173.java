...
public class Exam0173 {
  ...
  static class Car implements Cloneable {
    ...
    public Car clone() throws CloneNotSupportedException {
      return (Car) super.clone(); //어차피 상속받을 건데 원래의 클롲() 오버라이드한이유 - 접근범위를 퍼블릭으로 확장하기 위해서 오버라이딩 , 리턴타입 object 빵-소보로
    } //jvm 인스턴스 그대로 통째로 복제하므로 필드는 상관없어. 설계도면에 따라 일일이 필드를 일일이 만든다가 아님(상관없다), 통째로 복사 ., 메모리 통째로 복제, 서브클래스, 인스턴스 안따짐. 
  }
