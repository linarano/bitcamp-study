...
public class MyList {
  ...
  public Iterator iterator() {
    ...
    // local calss(로컬클래스)
    // 로컬클래스에도 바깥 클래스의 인스턴스 주소를 저장할 필드와 생성자가 자동으로 추가된다
    // 
    class ListIterator implements Iterator { //  로컬변수에 public 못붙이는 것처럼 로컬 클래스도 못붙인다. 
      ...
      public boolean hasNext() {
        ...
        return cursor < MyList.this.size();// 바깥클래스의 메서드호출
      }
      //if (cursor  >=  list.size()) {
      //  return false;
      // }
      ...
      @Override
      public Object next() {
        return MyList.this.get(cursor++); //  후위연산자  현재커서를 꺼내고, 나중에 커서 카운트 증가  //(MyList.this.)get(cursor++); 
      }
    }
