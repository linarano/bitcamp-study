...
public class Exam0110 {
  public static void main(String[] args) {
    ...
    // => 레퍼런스를 통해 호출하는 메서드는
    //    레퍼런스가 실제 가리키는 하위 클래스에서 찾아 올라 간다. A2 먼저 찾는다. (컴파일러는 레퍼런스타입만 보고 오케이)
    //
    // 그렇다고 해서 A2에서 추가한 메서드를 호출할 수는 없다.
    // => 즉 레퍼런스의 클래스를 벗어나서 사용할 수는 없다.
    //    컴파일러가 허락하지 않는다.
    //    a3.x(); // 컴파일 오류! - 실제무엇이 들어있는지  실행의 영역이므로 컴파일러는 관심없음// 이 레퍼런스를 가지고 이 메소드호출가능 불가능만 찾는다. 
