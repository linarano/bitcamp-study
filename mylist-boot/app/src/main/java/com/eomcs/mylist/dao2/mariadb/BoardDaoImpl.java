package com.eomcs.mylist.dao2.mariadb;
...
import com.eomcs.mylist.dao2.DaoException;
...
public class BoardDaoImpl implements BoardDao { // 스프링부트가 자동생성하고, 자동주입시켜버린다. BOardDao를 원하는 곳에
  ...
  @Autowired //=> 스프링 부트가 보관하고 있는  다음 타입의 객체가 있다면 주입해줄 것을 지시하는 애노테이션
  ...
  public int countAll() { // 인터페이스에 반드시 선언되어야함 예외가
    try (Connection con = dataSource.getConnection();  // 앱에 의존하게 되는 것 -> 하나라도 바뀌면 에러가뜬다 : 의존객체 자동으로 꼽아주는 기능을 쓰자 
        //DataSource에서 얻은 커넥션 객체는 close()할때 연결을 끊는 것이 아니라 DataSource에 반납된다. 
  ...
  public List<Board> findAll(){ // 규칙상 예외를 안던져
    try (Connection con = dataSource.getConnection();  
        PreparedStatement stmt = con.prepareStatement( //  트라이위즈 리소시스 안에 변수선언한게 아니므로 안닫힘
