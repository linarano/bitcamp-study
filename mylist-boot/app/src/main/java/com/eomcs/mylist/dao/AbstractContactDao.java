...
//반드시 추상클래스로 선언해야한다. 
//
public abstract class AbstractContactDao implements ContactDao { // 데이터 처리는 무조건 DAO에가 맡긴다. 
  ...
  @Override
  public int update(int no, Contact contact) throws Exception { // 나중에 업데이트하자 다른기능과함께 , 업데이트를 호출할 컨트럴러에게 예외를 던져라. 직접처리할지말고 
    if ( no < 0 || no >= contactList.size()) {
      return 0;
    }
