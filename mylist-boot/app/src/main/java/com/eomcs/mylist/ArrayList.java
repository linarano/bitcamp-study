...
public class ArrayList { // 같은 클래스는 굳이 클래스를 붙일 필요가 없다.
  ...
  //
  static Contact[] grow() {
    Contact[] arr = new Contact[newLength()]; // (레퍼런스)배열을 늘이는것.
    copy(contacts, arr);
    return arr;
  }
  ...
  // - 배열을 복사한다.
  // 
  static void copy(Contact[] source, Contact[] target) { // 기존배열, 새로 집어넣을 배열
  ...
  // 기능:
  // - 배열에 특정 위치에 값을 변경한다.
  // - 리턴 값은 변경하기 전에 저장되어 있던 값이다. (쓰던지말던지)
  static void set(int index, Contact contact) {
