...
public class Exam0222 {
  ...
  //위에는 다가능
  //밑에   add가 문제임
  static void m1(ArrayList<?> list) { //0211예제와 동일한 내용
    ...
    // 즉 레퍼런스의 지정된 제네릭 타입으로만 유효여부를 검사한다.
    //
    // 위의 list 파라미터는 ArrayList 클래스의 제네릭 타입이 ?로 설정되어 있다. *****
    // 즉 ArrayList의 타입 파라미터가 설정되어 있지 않기 때문에 ****
    // 따라서 add() 메서드의 파라미터 타입이 뭔지 알 수 없다.(해당 메서드의 파라미터 타입이 ? 된것: 정해지지않아서) -> 컴파일러가 검사할 수가 없다. 
    ...
    Object obj1 = list.get(0); // 이건 보증해준다. 왜냐면 리턴타입이 따질필요없이 다 Object 레퍼런스에 저장할 수 있으므로
