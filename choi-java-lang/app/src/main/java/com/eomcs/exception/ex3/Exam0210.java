...
public class Exam0210 {
  ...
  //    => JVM에서 발생된 오류이다.
  //    => 개발자가 사용하는 클래스가 아니다.
  //    => 이 오류가 발생하면 현재의 시스템(어플리케이션) 상태를 즉시 백업(현재 어플리케이션에서 작업했던 메모리값들을 임시파일에 저장하고 즉시종료)하고, 실행을 멈춰야 한다.
  //    => JVM에서 오류가 발생한 경우에는 계속 실행해봐야 소용이 없다. => 즉시 저장하고 종료해라. 우리가 할 수 있는게 없다. 
  //       근본적으로 문제를 해결할 수 없다.
  //    => 오류의 예:
