...
public class Server0110 {
  public static void main(String[] args) throws Exception {
    ...
    ServerSocket ss = new ServerSocket(8888); //   서버쪽 포트번호는 개발자가 지정.
    // 포트번호
    ...
    // 잠깐 멈추기 - 종료하지말라는 의미 - 키보드에서 한줄이 입력될때까지 기다려 (지금은 무한루프를 돌리지않으므로)
...
//랜카드로 데이터가 들어오면 8888포트로 들어오는 프로그램이 3개면 누구한테 주나

//그래서 포트번호는 중복되면 안ehla
//클라이언트든 서버든 프로그램을 구분할 때 사용할 포트번호는 반드시 있어야한다.