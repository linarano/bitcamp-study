...
public class Exam01 {
  ...
  public static void main(String[] args) {
    ...
    // 객체 생성한다는 건
    // 객체필드값 사용조회변경 을 하겠다는의미
    // 그 객체에 메서드사용 - 인스턴스필드는 메모리에, 메서드는 메서드아레아
    // 객체 용어를 다양하게 쓰인다. 인스턴스 또는 클래스(문맥에 따라서 해석해라.)
    //객체(클래스)마다 메서드 시그너처가 다르다. = 파라미터순서, 파라미터,리턴타입
    ...
    // 메서드 시그너처가 다르기 때문에 
    // 호출할 때 일관성이 없다. (클래스마다 비슷한 기능을 하는데, 객체에 정의된 메서드의 호출방식이 다르다 - 일관성이 없으므로 유지보수가 어렵다.)
