...
public class Board implements Serializable {
  private static final long serialVersionUID = 1L; //바이너리데이터로 보낼 수있도록 허락함

  // DB 테이블의 컬럼 값을 저장할 인스턴스 변수를 준비한다.
  // => 보통 컬럼이름은 DB 관례에 따라 약자로 기술한다.
  // => 그러나 자바에서는 자바의 관례에 따라 변수명을 만들라!
  //    DB 컬럼명과 같게 하지 말라!
  //자바 변수명에 _ 적지마라. int board_id // 데이터베이스쪽을 먼저 만진사람이 그럼
  //특정 dbms에 종속되게 짓지마라 -> 자바에 따라 dbms를 여러개 쓸 수 있음 
  int no;
  String title;
  String content;
  Date registeredDate;
